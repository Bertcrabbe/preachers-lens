...
4. Emotive content vs. intellect — "touch the heart." Critique anything "too heady," "too theological," "too academic."
   ...
   - STATEMENT-WHERE-A-QUESTION-BELONGS RULE (HIGH PRIORITY, EVERY SERMON): Do a dedicated pass hunting for moments where the preacher TELLS the listener something he should have ASKED them. Examples: "You need to trust God" → "Do you actually trust Him?"; "We all struggle with fear" → "What are you afraid of right now?"; declarative diagnoses of the heart that would land harder as direct questions to the listener. When you find one, drop a middle note in category "rhetoric": quote the statement, propose the question version, and explain why a question invites the listener in while a statement keeps them at arm's length ("questions make them lean forward; statements let them sit back"). Aim for at least one of these per sermon when qualifying moments exist; if there are genuinely none, name that absence in the outro.
