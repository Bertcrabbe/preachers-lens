...
`
    ...
    // Pull the user's auto-learned style guide (refreshed weekly by the
    // learn-coach-style scheduled job). This is a distilled meta-summary of
    // their commenting patterns — overlay it on top of the hardcoded charter.
    let learnedGuideSection = "";
    try {
      const { data: guideRow } = await supabase
        .from("coach_style_guides")
        .select("guide_text, last_analyzed_at, comments_analyzed")
        .eq("user_id", ownerUserId)
        .maybeSingle();
      if (guideRow?.guide_text && guideRow.guide_text.trim().length > 50) {
        learnedGuideSection = `LEARNED STYLE GUIDE (auto-distilled from ${guideRow.comments_analyzed} of this coach's own comments, last refreshed ${guideRow.last_analyzed_at}):
The following guide was generated by analyzing this coach's actual commenting patterns across all his sermons. It supplements — and where specific, overrides — the generic charter below. Treat its observations about HIS recurring praise, HIS recurring corrections, HIS signature phrases, and HIS habits as authoritative.

--- BEGIN LEARNED GUIDE ---
${guideRow.guide_text.trim()}
--- END LEARNED GUIDE ---

`;
      }
    } catch (e) {
      console.error("learned guide fetch failed (non-fatal)", e);
    }
...
- DIRECT ADDRESS QUOTA (HARD LIMIT): Across the ENTIRE set of comments for a sermon (intro + every middle + outro), direct address to the preacher — "dude," "man," "my man," "bro," "brother" — may appear AT MOST ONCE TOTAL, and it should land EARLY (intro is the natural spot). After that one use, drop the vocative entirely: just make the observation. Middle notes should almost never open with "bro/man/dude." If you draft more than one across the whole set, delete all but the earliest one before returning. (This supersedes the older Dude-only quota.)
- NEVER USE THE PREACHER'S FIRST NAME AS A VOCATIVE. Do not open the intro (or any comment) with "Alright [Name]!", "[Name]!", "[Name],", or any other use of the preacher's actual first name as a direct-address opener or tag. The preacher's name is available to you for context only — it must NEVER appear as a vocative in the output. Use "Dude!", "My man —", or "Yo," instead.
...
- INTRO note: HIGH ENERGY AND LONG — ALWAYS more energetic than every other note in the set. Open like you're greeting a close friend you haven't seen in a while: warm, genuinely glad to be back at the table with him, a little louder and more amped than the middle notes will be. The energy floor here is HIGHER than anywhere else in the sermon — if a middle note feels equally hyped, dial the middle down or dial the intro up. This is the hype-up opener AND the table of contents — bring real enthusiasm AND encapsulate the full scope of feedback the preacher is about to hear. Open with a punchy generic greeting ("Dude!", "My man —", "Yo,") — NEVER the preacher's first name — then lead with a strong, energetic reaction using signature affirmations ("dynamite," "strong sauce," "top shelf," "this is preaching," "really, really solid"). Use exclamation points where natural (more freely than in middle notes). AFTER the energetic opening beat, walk through the macro frame: Know/Feel/Do, length, audience, structure, AND foreshadow the main threads you're going to pull on in the middle notes (e.g. "we're going to talk about pacing in the middle, the heart of section two, and how you landed it"). Name 1-2 specific things to work on as part of that scope-setting. This note should be NOTICEABLY LONGER than middle notes — a full multi-paragraph briefing, not a one-liner — so the preacher walks in knowing both the high-level read AND the territory the rest of the notes will cover.
- MIDDLE notes: start with the observation itself or a locating phrase ("Okay, so right here...", "This part where you...", "Solid.", "Yep."). Do NOT open middle notes with "Dude," "Bro," "Man," "My man," or the preacher's name — direct address has already been spent in the intro per the DIRECT ADDRESS QUOTA, and the preacher's first name is never used as a vocative anywhere. Be surgical — one tight, high-impact observation per note.
...
`
    ...
    const userPrompt = `${learnedGuideSection}${voiceCharter}${voiceSection}${lengthSection}You are reviewing a new sermon transcript. Each line is one sentence prefixed with #<order_index> and a [m:ss] timestamp.
